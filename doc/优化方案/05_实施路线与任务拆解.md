...
| 3 | 断网失败时延 ≤ connect_timeout+1s | 🟡 部分 | 单次尝试实测 <1.5s（端口关闭 `single_attempt_against_closed_port_is_fast`）；但用户可见最坏还要加 2 次重试 + 2.4s 退避，已在同一测试里量化 |
...
| 13 | 崩溃后配置损坏率 0% | ✅ 已证 | 真子进程 SIGABRT 故障注入（`crash_between_tmp_and_rename_leaves_previous_file_intact`），并用 mutation 证明注入点确实被走到 |
| 14 | capabilities 里 shell 权限 0 | ✅ 已证 | 静态审计：只剩 `core/clipboard-manager/dialog` |
| 15 | 5 并发下 UI 无可感知卡顿 | ✅ 已测 | 原生壳内 5 个并发任务全部 settled，期间 26 帧、**最大帧间隔 21.0ms**（≈47fps 下限），无长阻塞 |
...
| 16 | CI 门禁执行且绿 | ❌ 未执行（需 push） | `quality.yml` 已写、其内每条命令本地逐条跑通，但从未在 GitHub 上真正执行（需提交并推送） |
