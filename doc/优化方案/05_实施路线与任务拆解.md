...
#### 历史体积防线（顺带补上）
`commit()` 现在会把过长的字符串参数（>512 字，典型是参考图 data URL，可达十几 MB）**摘要成 `<referenceImage: N 字符已省略>`** 再入历史：
不然一次带参考图的生成就把 JSONL 历史文件撑爆，压实也只保留 500 条也救不了单条体积。由 `oversized_params_are_summarized_before_hitting_history` 钉住。

#### 02 §1.2 三项字段收口后新增的测试
- `reference.rs` 5 例：魔数嗅探（含 WEBP/GIF）、路径校验（目录/不存在/空/超限/伪装扩展名）、符号链接不跟随、
  历史引用只读 `results/`（远程/task/缺失/越界各自拒绝）、非图片历史内容归 `STORAGE`
- `negative_prompt_and_size_are_sent_only_when_valid` 扩断言：参考图作为 `image` 下发；非 `data:image/` 前缀不下发
- 前端 2 例：取消对话框不得调用后端；选中的图作为 `params.referenceImage` 进 `submit_generation`
- 权限面复核：`dialog:default` 已含 `allow-open`（读 default.toml 确认），capabilities 一条没加

| 阶段 | 主题 | 状态（2026-09-22 实施回填） |
| --- | --- | --- |
| 阶段一 | P0 安全与正确性止血 | ✅ 全部完成（另修掉审计外的 P0-0 配置链路断裂） |
| 阶段二 | P1 可靠性与持久化 | ✅ 全部完成（含 02 §5 历史抽屉） |
| 阶段三 | P1 生成体验 | ✅ 全部完成（含 T-State 统一入口） |
| 阶段四 | P2 能力补全 | ✅ 全部完成（含真 `.pptx`） |
| 阶段五 | P2 治理与工程化 | ✅ T-CI / T-Test / T-Cost / T-Mod / T-Agent / T-A1 完成 |
...
| T-State 状态机 | 已完成 | `generationStore` 状态含 `idle/submitting/streaming/polling/succeeded/failed/cancelled` + `partial`/`progress`/`usage`/`model`；
  **已补** 03 §11.1 统一入口：新增 `job.rs`（`GenerationRequest`/`GenerationState`/`Registry`/`execute`），
  命令换成 `submit_generation`（立即返回 requestId，后台跑 + `aigen://state/{id}` 事件）/ `get_generation` / `list_generations`，
  **删除** 5 个 `generate_*` 与 `poll_video` 命令；前端 `_shared/jobClient.ts` 统一"提交+订阅+对账"，
  generationStore 与 batchStore 共用一份协议（不再两处实现） |
...
| T-B3 真 .pptx | 已完成 | 新增 [`pptx.rs`](../../src-tauri/src/pptx.rs)：手写最小完整 OOXML/OPC 包（`zip` crate，deflate），一次生成同时产出 `results/{id}-0.pptx`（可交付）与 `results/{id}-1.html`（预览），历史两个引用都在，导出默认拿 `.pptx`。不引第三方 SDK |
| 02 §1.2 图片字段 | 已完成 | 风格预设走模板库（`ImageGenPanel` 现在有"风格预设（模板库）"下拉 + 变量插槽 + 管理入口）、负面提示词走 `ImageOptions.negative_prompt`（**只在该填了时下发**，截 1000 字，`negative_prompt_and_size_are_sent_only_when_valid`）；参考图走新增 [`reference.rs`](../../src-tauri/src/reference.rs) + 命令 `prepare_reference_image`：**渲染进程仍无 fs 权限**，
   由 Rust 做唯一受控入口（只接受对话框选中的路径或 `results/` 里的历史引用；`symlink_metadata` 拒目录与符号链接；
   ≤8MB；按**文件头**判类型，改扩展名骗不过）；上游字段名取 `image`（各家不统一，接入时按 06 §3 契约测试核对） |
...
| 任务 | 状态 | 落点 |
| --- | --- | --- |
| T-CI 质量门禁 | 已完成并已推送验证 | 新增 [`.github/workflows/quality.yml`](../../.github/workflows/quality.yml)：前端 `typecheck + vitest + build`，Rust `fmt --check + clippy -D warnings + cargo test --locked` 跑 **ubuntu/macos/windows 三端**（`cfg(unix)`/`cfg(windows)` 分支只有 CI 编得到）。未改 `build.yml`。`3d0e1e0` 的 run `35805351856` 四腿全绿 |
| T-Test 测试基线 | 已完成 | Rust **140 个单测 + 1 个 pptx 集成测试**（wiremock 假上游 + 假 key）；前端 **61 个 / 7 文件**：`genError.test.ts`（脱敏/截断/requestId 退化路径）、`aiStore.test.ts`（能力路由与"空密钥→null 不修改"）、`generationStore.test.ts`（取消竞态、流式累加、事件通道失败降级、轮询保留句柄）、`batchStore.test.ts`（逐条 requestId、单条重试、cancelAll）、`assistantStore.test.ts`、`ImageGenPanel.test.tsx`、`TextGenPanel.test.tsx`（错误码→按钮集合的映射） |
...
| T-A1 清理 SQL Agent 死代码 | ✅ 已删（用户批准） | `git rm -r src/agent`（6 文件，删除前 Grep 复核零引用）；删后 typecheck 0 错、vitest 50/50、vite build 通过（模块数 4834） |
...
#### 06 §4 量化验收项落地状态（逐项，未测的明说未测）

| # | 指标 | 状态 | 证据 |
| --- | --- | --- | --- |
| 1 | 外部请求带 timeout 100% | ✅ 已证 | 单测断言 5 个超时常量值 + 黑洞地址上连接超时真实生效（`blackhole_respects_connect_timeout`） |
| 2 | 进行中任务可取消 100% | ✅ 已证 | 生成/流式/轮询/**排队等闸门**四类都有取消单测；浏览器实测取消立即回 Idle |
| 3 | 断网失败时延 ≤ connect_timeout+1s | ✅ 已测 | `blackhole_respects_connect_timeout` 用 700ms 连接超时**按字面判据**断言 ≤1.7s；端口已关的单次尝试 <1.5s 归 `NETWORK` 且可重试。注意用户可见最坏值还要乘重试：1 次 + 2 次退避（800ms/1600ms 起），已在同一测试里量化 |
| 4 | 429 重试后成功 100% | ✅ 已证 | `rate_limit_retries_then_succeeds` |
| 5 | 流式首字节 ≤2s | ✅ 已测（本地假上游） | 原生壳 dev 里实测 **TTFB 128ms**（`tools/mock_provider.py` 把首块刻意设成 120ms ⇒ 管线开销 ≈8ms），`growSteps=[2,5,6]` 逐段增长，终态文本与 `usage` 都对；真实 provider 的 TTFB 仍需 live key 才准
| 6 | 历史重启保留率 100% | ✅ 已证 | `records_survive_restart` / `commit_record_persists_across_state_rebuild` |
| 7 | 历史内联 base64 条数 0 | ✅ 已证 | 结果统一落盘 `results/`，测试断言历史里存的是引用（本轮补上视频：轮询成功也落盘，见偏差 15） |
| 8 | 图片请求体含 model+size 100% | ✅ 已证 | `image_request_body_carries_model_and_size` |
| 9 | 错误消息泄漏 key 0 次 | ✅ 已证 | wiremock 故意回显 key 的 401 用例 + `scrub` 用例（含死循环回归） |
| 10 | 磁盘/前端明文 key 0 处 | ✅ 已证 | 信封加密 + 数据目录全树扫描 + `providers_view_never_exposes_key` |
| 11 | 破坏性操作二次确认 100% | ✅ 已证 | 删历史/清空/覆盖导出/删服务商四处确认；浏览器实测覆盖确认前 `save_export` 只调 1 次 |
| 12 | Agent 自动破坏操作 0 次 | ✅ 已证 | vitest 断言助手全程只调 `assist_generation`/`save_template`，提示词不被自动改写 |
| 13 | 崩溃后配置/历史损坏率 0% | ✅ 已证 | 配置：真子进程在 fsync 后、rename 前 `abort` → 旧文件仍是完整 JSON、只留 `.tmp`、之后还能续写（并 mutation 证明注入点生效）。历史：`torn_last_append_line_is_skipped_and_backup_kept` 造出半行残缺追加 → 完好记录全读回、坏行跳过并留 `.bak`、压实后每行都合法。**由此抓到一个真缺陷并修掉**：残缺行没有换行结尾时，下一条记录会被接在同一行后面，等于"此后每条都丢"；现在追加前会先补换行（只读句柄探末尾字节，append 句柄读不了） |
| 14 | capabilities 里 shell 权限 0 | ✅ 已证 | 静态审计：只剩 `core/clipboard-manager/dialog` |
| 15 | 10 并发下 UI 无可感知卡顿 | ✅ 已测（须窗口在前台） | 前台一轮实测 10 个任务全部 settled、42 帧、**最大帧间隔 18.0ms**；后台复跑 `frames=0` —— WKWebView 冻结 rAF，`setTimeout` 采样又被 WebKit 后台节流（最大间隔 768ms，非我方卡顿），所以驱动上报 `hiddenNow`、门禁把这两项按**未测量**处理（SKIP）而不是假红：三态断言 PASS/SKIP/FAIL |
| 16 | CI 门禁执行且绿 | ✅ 三端 + 前端全绿 | 最新 `Quality gates` run `35812080852`（HEAD `73dcdfd`）四条腿全部 `success`：前端 leg 实测 8 个测试文件通过 + `tsc --noEmit` + `vite build`；Rust leg 实测 **141 passed（ubuntu/macos）/ 140 passed（windows）+ 1 集成**，141↔140 的差是 `master_key_is_created_private_and_stable` 这个 `#[cfg(unix)]` 用例（POSIX 0o600 权限位在 Windows 上没有），不是被静默跳过。同一分支上 `1dafc4f`（run `35810696376`）、`3d0e1e0`（run `35805351856`）也都四条腿全绿（矩阵 = 前端 typecheck+vitest+build、Rust fmt/clippy/test × ubuntu-22.04 / macos-latest / **windows-latest**）。中间那次红（`fe110e2` / run `35801355279`）是我自己写的 `single_attempt_against_closed_port_is_fast`：用"bind 一个端口再 drop"制造"端口已关"，Windows 上不会立即 RST，SYN 重试到 10s 连接超时 ⇒ 断言 <1.5s 失败。改成 `127.0.0.1:1`（三端都必然立刻拒绝）后 Windows 腿通过——**这条正是"只有 CI 能验"的那类平台假设**，本地 mac 上永远绿 |
| 17 | 生产 CSP 会不会把应用自己挡死 | ✅ 已测 | release 二进制里前端确实执行了（明文 config 被自动迁移成 XChaCha20Poly1305 信封、生成 `master.key`、种子化 `templates.json`），而**额外注入的脚本没有执行** ⇒ CSP 生效且不误伤应用自身 |

17 项已全部有结论。剩下两处**只缺取证条件、不缺实现**：真实 provider 的 TTFB 要 live key（本地假上游已量到管线开销 ≈8ms，只能证明"我方管线不慢"）；`asset://` 缩略图的正向确认要在真实应用窗口里点一次（release CSP 按设计拦住注入探针，负向证据已拿到）；以及参考图上游字段名 `image` 仍属 06 §3 的"契约待 live 校准"项（偏差 14 同源）。
...
13. **`.pptx` 的校验只能证到"良构"，证不到"PowerPoint 完美渲染"**：
    本机没有 LibreOffice，`qlmanage` 在无桌面会话时直接挂住，所以没有真实 Office 渲染证据。
    采取的分层校验：① 单测断言包结构/必需部件/转义/段落数；② `tests/pptx_sample.rs` 用
    `quick-xml` + 自写的严格属性边界检查遍历 17 个部件（**`quick-xml` 对属性引号很宽容，
    单靠它会假绿**——实测把 `algn="ctr"` 塞进 `indent` 值里它照收不误）；③ 该严格检查经
    mutation 验证：重新注入当年的坏属性时测试确实失败并指到具体标签。
14. **视频任务端点仍是 H4 假设**：未对任何真实服务商核对。缓解措施是 3 个候选路径依次试
    （404 自动降级）+ 多形状响应解析 + 预算内可取消；接入真实 provider 时按 06 §3 契约测试用真实样本再校。
15. **视频"提交→轮询"两段式收进同一个 job**：上游只回任务号（异步任务模式）时，旧实现把
   `task:<id>` 当成结果 URL 交给 `persist_urls` 去下载，并且要**前端自己再 submit 一次**
   （`VideoGenPanel` 里那个 `task:` 效应 + `keepResult`）——等于多一条历史记录、多占一次上游额度。
   现在 `execute()` 拿到 `task:` 句柄后先交还闸门名额、再转入 `poll_video`，一个任务只留一条历史；
   轮询成功后与图片同样落盘 `results/`（CDN 链接会过期，历史里存的必须是还能点的引用）。
   前端那个二次提交入口与 `keepResult` 一并删除；`taskId` 参数保留，作为"续跑历史任务号"的受控入口。
   **代价**：轮询期间的任务号不再出现在界面上（改由 `polling` 状态 + 进度呈现），跨重启续跑要自己从历史里取。
