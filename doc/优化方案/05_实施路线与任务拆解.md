...
| 7 | 历史内联 base64 条数 0 | ✅ 已证 | 结果统一落盘 `results/`，测试断言历史里存的是引用 |
...
| 16 | CI 门禁执行且绿 | 🟡 已执行、当前红 | `Quality gates` 已在 GitHub 跑过两次：`23ba358` 三端全绿；`fe110e2`（job/pptx 那版）**Windows 腿红**——失败的是我自己写的 `single_attempt_against_closed_port_is_fast`：用"bind 一个端口再 drop"来制造"端口已关"，Windows 上不会立即 RST，SYN 重试到 10s 连接超时 ⇒ 断言 <1.5s 失败。已改成 `127.0.0.1:1`（三端都必然立刻拒绝）并在此注释里记住原因；本地 140 测全绿，**待 push 复验**

未测的三项（5/15/16）都不是"实现缺失"，而是**取证环境缺失**：5 需要 live 上游，15 需要可见视口，16 需要一次 push。
